...
kton=read.csv("R_project/R_tem_project/karlatonitte24.csv")          # All CSV files are Invoked Here
...
main = function() {
  ...
  repeat {
    ...
    cat("Enter AM/PM :")
    f=readline()  
    #taking all the inputs like form and to place and time with AM or PM
    if((f[[1]]=="PM"||f[[1]]=="Pm"||f[[1]]=="pm") & z<strptime("12:00",format="%H:%M")){
      z=z+43200
    }                                                                                                   
    else if(z>=strptime("12:00",format="%H:%M") & f[[1]]=="AM") z=z-43200  
    #if it is afternoon then we will convert it into 24hr pattern else if it is greater 
    #than or equal to 12:00 at morning convert it into 24 hr pattern
    if ((obj2 - obj1) >= 0) {
      i=obj1
      while (i < obj2) {
        print(x[1,i][[1]])
